...
team already names in the slides (FORT, SAGE, and design conditioning).
The action list below frames the fixes against those labels.

1. **(Addresses #1, "design-conditional residuals" upgrade)** Move the first-stage regression to a log-log form with a small set of coarse design indicators (within vs. between subjects; clustered vs. non-clustered; experimental vs. observational): `ln(SE_i) = α + β ln(N_i) + Γ'D_i + ν_i`. Reinterpret residuals as "precision unexplained by N **and design**." Then validate that negative residuals correlate with p-hacking markers (significance bunching, post-hoc covariate selection) rather than markers of honest design. Keep the design vector deliberately small to avoid weak-IV and collinearity with N.
2. **(Addresses #2, "FORT" upgrade)** Replace the fixed exponential tilt `ω_i = exp(−max(0, −π_i))` with a data-driven robust scheme — iteratively reweighted least squares with a Huber or Tukey bisquare loss, or MM-estimation, on the second-stage PEESE regression. Let the data choose the tilt parameter `δ`. If clean studies dominate, `δ ≈ 0` and WAIVE collapses to MAIVE; if spurious-precision studies are present, the robust weighting downweights them automatically. This is what the WAIVE slides label "Fitted Outlier-Robust Tilt"; the criticism is that the slides assert the tilt without fitting it.
3. **(Addresses #3)** Add a head-to-head MAIVE-vs.-WAIVE ablation panel using the same simulation grid and empirical sample as the MAIVE *Nature Comms* paper. **First check whether slides 22-26 already include this; if so, the objection partially resolves.**
4. **(Addresses #4)** State explicitly why the downweight is one-sided (likely an incentive argument: researchers chase significance, so misspecification that reduces SE is more common than misspecification that inflates it), or symmetrize.
5. **(Addresses #5 — minority report)** Justify the two-step usage of `π_i`: explain why a residual that MAIVE *removes* from precision should also *reduce influence* in the second stage. The argument needs to be that negative `π_i` predicts biased estimates *after* MAIVE adjustment, not merely biased reported variances *before* it. If the argument cannot be made cleanly, reframe WAIVE as a single-stage robust estimator rather than a two-stage MAIVE-plus-tilt construction.
6. **(Addresses #6, "SAGE" upgrade)** Build a GMM extension of MAIVE/WAIVE with multiple functions of N as instruments — e.g., `Z_i = (1/N_i, 1/√N_i, ln N_i)` — and report the Hansen J-test for overidentifying restrictions. **Interpretation rule:** J-rejection is *informative*, not *automatically fatal* (heterogeneity, nonlinearity, and misspecification can also trigger it). The right operational rule is: if J rejects strongly, flag WAIVE as unsafe for that dataset and fall back to MAIVE plus robust RE; if J does not reject, present WAIVE alongside MAIVE as a serious robustness check. This is what the WAIVE slides label "SAGE"; the criticism is that the diagnostic should be the headline, not the appendix.
7. **(Framing)** In the next presentation/paper, label WAIVE as "an experimental sensitivity layer on MAIVE" rather than "a successor estimator" until the ablation in #3 and the diagnostics in #6 are published. The current slides already include this caveat verbally ("still just a concept; in the meantime, use MAIVE"); make it the headline.

**A unified "WAIVE 2.0" workflow** that addresses all six criticisms in one pipeline would look like: design-conditional log-log first stage (criticism #1) → MAIVE point estimate as baseline → FORT-style robust second stage (criticism #2) → SAGE-style GMM diagnostics (criticisms #5, #6) → ablation panel reporting MAIVE, WAIVE-FORT, and selection-model alternatives side by side (criticism #3). Always report AR confidence intervals.
